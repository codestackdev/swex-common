...
{
    [TestClass]
    public class SldWorksCommonExTests
    {
        [TestMethod]
        public void IsVersionNewerOrEqualTest()
        {
            var sw2017sp23Mock = new Mock<ISldWorks>();
            sw2017sp23Mock.Setup(m => m.RevisionNumber()).Returns("25.2.3");
            var app = sw2017sp23Mock.Object;
            ...
            Assert.IsTrue(r6);
            Assert.IsFalse(r7);
            Assert.ThrowsException<ArgumentException>(() => app.IsVersionNewerOrEqual(SwVersion_e.Sw2017, null, 1));
        }
    }
}
